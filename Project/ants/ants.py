...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        place = self.place
        count = 0
        while place is not None and not place.is_hive and count <= self.upper_bound:
            if count >= self.lower_bound and place.bees:
...
# The WallAnt class
class WallAnt(Ant):
    """WallAnt is a strong ant that does not attack, but blocks the path."""
...
# The HungryAnt Class
class HungryAnt(Ant):
    """HungryAnt eats a Bee in its Place when it expires."""
    ...
    def action(self, gamestate):
        """A HungryAnt's action checks if it can eat a random Bee in its Place."""
        if self.cooldown > 0:
            self.cooldown -= 1
        else:
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 8a
        "*** YOUR CODE HERE ***"
        return self.ant_contained is None if isinstance(other, Ant) and not isinstance(other, ContainerAnt) else False
        # END Problem 8a
...
class TankAnt(ContainerAnt):
    """The BodyguardAnt provides great defense, but they say the best defense is a good offense."""
    name = 'Tank'
    food_cost = 6
    implemented = True
    damage = 1
    ...
    def action(self, gamestate):
        ...
        while place is not None:
            if place.ant is not None:
                place.ant.double()
            if isinstance(place.ant, ContainerAnt) and place.ant.ant_contained != None:
                ant = place.ant.ant_contained
                ant.double()
            place = place.exit
        # END Problem 12
