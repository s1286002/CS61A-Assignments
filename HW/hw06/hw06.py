...
def store_digits(n):
    ...
    if n < 10:
        ...
        # Append the last digit to the end
        current = rest
...
def deep_map_mut(func, s):
    ...
    if s is Link.empty:
        ...
        if isinstance(s.first, Link):
            # If the first element is a Link, apply deep_map_mut recursively
            deep_map_mut(func, s.first)
        else:
            # Otherwise, apply the function to the first element
            s.first = func(s.first)
        # Recur on the rest of the link
